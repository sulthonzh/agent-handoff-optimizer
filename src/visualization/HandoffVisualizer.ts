...
export class HandoffVisualizer extends EventEmitter {
   ...
   */
  toMarkdown(data: PerformanceMetrics): string {
    const lines: string[] = [];
    const s = data.systemMetrics;
    const agents = Object.entries(data.agentMetrics);
    const handoffs = data.handoffMetrics;
    const duration = ((data.timeWindow.end - data.timeWindow.start) / 1000).toFixed(1);
    ...
    if (handoffs.length > 0) {
      const routes: Record<string, { count: number; totalLatency: number }> = {};
      for (const h of handoffs) {
        const key = h.fromAgent + ' → ' + h.toAgent;
        if (!routes[key]) routes[key] = { count: 0, totalLatency: 0 };
        routes[key].count++;
